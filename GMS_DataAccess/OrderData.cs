	...
	public class OrderData
	{
		public static DataTable getOrders(string searchString)
		{
			...
			if (!string.IsNullOrEmpty(searchString) && searchString.Contains("-"))
				return CRUD.getUsingDateTable($"select o.Id 'OrderId', o.Date, u.UserName, CONCAT(per.FirstName ,' ', per.LastName) 'Supplier' , o.TotalAmount, o.Discount from Orders o join Users u on o.UserId = u.Id join Suppliers s on o.SupplierId = s.Id join Persons per on s.PersonId = per.Id  where o.Date ='{searchString}'");
			else
				return CRUD.getUsingDateTable($"select o.Id 'OrderId', o.Date, u.UserName, CONCAT(per.FirstName ,' ', per.LastName) 'Supplier' , o.TotalAmount, o.Discount from Orders o join Users u on o.UserId = u.Id join Suppliers s on o.SupplierId = s.Id join Persons per on s.PersonId = per.Id  where lower(u.UserName) like '%{searchString}%' or lower(per.FirstName) like '%{searchString}%' or lower(per.LastName) like '%{searchString}%' order by date desc");
		}
